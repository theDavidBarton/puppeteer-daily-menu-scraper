...
async function testFlight() {
  ...
  */
  ...
  const outboundDuration =
    "div.outbound > div.timing > div.center > div.duration";
  ...
  */
  ...
  let airFromContent = await page.evaluate(
    el => el.value,
    await page.$(airFrom)
  );
  ...
  */
  ...
  let airFromDateContent = await page.evaluate(
    el => el.innerText,
    await page.$(airFromDate)
  );
  await page.waitForSelector(airToDate);
  let airToDateContent = await page.evaluate(
    el => el.innerText,
    await page.$(airToDate)
  );
  ...
  airFromDateContent = await page.evaluate(
    el => el.innerText,
    await page.$(airFromDate)
  ); // format: 18 Avr. 2019 (jeudi)
  airToDateContent = await page.evaluate(
    el => el.innerText,
    await page.$(airToDate)
  ); // format: 25 Avr. 2019 (jeudi)
  ...
  let airFromDateSelectedDay = await page.evaluate(
    el => el.innerText,
    await page.$(actualDate)
  );
  let airFromDateSelected =
    airFromDateSelectedDay + " " + airFromDateSelectedMonth; // format: 18 Avril, 2019
  ...
  console.log(
    "        √ departure field contains the selected day: " +
      airFromDateSelectedDay
  );
  ...
  let airToDateSelectedDay = await page.evaluate(
    el => el.innerText,
    await page.$(actualDate)
  );
  ...
  console.log(
    "        √ destination field contains the selected day: " +
      airToDateSelectedDay
  );
  ...
  */
  ...
  let departureHeaderContent = await page.evaluate(
    el => el.textContent,
    departureHeaderSelector
  );
  let arrivalHeaderContent = await page.evaluate(
    el => el.textContent,
