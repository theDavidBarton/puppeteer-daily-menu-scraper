...
describe('price catcher', function() {
  ...
  test('should catch the 2nd price from the string', function() {
    textContentTest = priceCatcher.priceCatcher(mockedContent[7], 1)
    expect(textContentTest).toBe('1200')
  })
  test('should not break function if input is empty (undefined)', function() {
    textContentTest = priceCatcher.priceCatcher(mockedContent[8]) // there is no such element of the object : undefined
    expect(textContentTest).toBe('n/a')
  })
})
